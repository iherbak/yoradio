...
bool NetServer::begin(bool quiet)
{
  ...
  if (config.emptyFS)
  {
    webserver.on("/", HTTP_GET, [](AsyncWebServerRequest *request)
                 { request->send_P(200, "text/html", emptyfs_html, processor); });
  ...
  else
  {
    ...
    webserver.on("/webboard", HTTP_GET, [](AsyncWebServerRequest *request)
                 { request->send_P(200, "text/html", emptyfs_html, processor); });
    webserver.on("/webboard", HTTP_POST, [](AsyncWebServerRequest *request)
                 { request->redirect("/"); }, handleUploadWeb);
  }
  ...
  if (IR_PIN != 255)
    webserver.on("/ir", HTTP_GET, handleHTTPArgs);
...
void NetServer::processQueue()
{
  ...
  if (xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS))
  {
    ...
    case GETMODE:
      sprintf(wsbuf, "{\"pmode\":\"%s\"}", network.status == CONNECTED ? "player" : "ap");
      break;
    case GETINDEX:
      ...
      break;
    case GETSYSTEM:
      sprintf(wsbuf, "{\"sst\":%d,\"aif\":%d,\"vu\":%d,\"softr\":%d}", config.store.smartstart != 2, config.store.audioinfo, config.store.vumeter, config.store.softapdelay);
      break;
    case GETSCREEN:
      sprintf(wsbuf, "{\"flip\":%d,\"inv\":%d,\"nump\":%d,\"tsf\":%d,\"tsd\":%d,\"dspon\":%d,\"br\":%d,\"con\":%d}", config.store.flipscreen, config.store.invertdisplay, config.store.numplaylist, config.store.fliptouch, config.store.dbgtouch, config.store.dspon, config.store.brightness, config.store.contrast);
      break;
    case GETTIMEZONE:
    ...
    case GETWEATHER:
      ...
      break;
    case GETCONTROLS:
      sprintf(wsbuf, "{\"vols\":%d,\"enca\":%d,\"irtl\":%d}", config.store.volsteps, config.store.encacc, config.store.irtlp);
      break;
    case DSPON:
    ...
    case STATION:
      ...
      break;
    case STATIONNAME:
      sprintf(wsbuf, "{\"nameset\": \"%s\"}", config.station.name);
      break;
    case ITEM:
      sprintf(wsbuf, "{\"current\": %d}", config.store.lastStation);
      break;
    case TITLE:
    ...
    case NRSSI:
      ...
      break;
    case SDPOS:
      sprintf(wsbuf, "{\"sdpos\": %d,\"sdend\": %d,\"sdtpos\": %d,\"sdtend\": %d}", player.getFilePos(), player.getFileSize(), player.getAudioCurrentTime(), player.getAudioFileDuration());
      break;
    case SDLEN:
      sprintf(wsbuf, "{\"sdmin\": %d,\"sdmax\": %d}", player.sd_min, player.sd_max);
      break;
    case SDSNUFFLE:
      sprintf(wsbuf, "{\"snuffle\": %d}", config.sdSnuffle);
      break;
    case BITRATE:
      sprintf(wsbuf, "{\"bitrate\": %d}", config.station.bitrate);
      break;
    case MODE:
...
void NetServer::onWsMessage(void *arg, uint8_t *data, size_t len, uint8_t clientId)
{
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT)
  {
    ...
    if (config.parseWsCommand((const char *)data, cmd, val, 65))
    {
      ...
      if (strcmp(cmd, "newmode") == 0)
      {
        newConfigMode = atoi(val);
        requestOnChange(CHANGEMODE, 0);
        return;
      }
      if (strcmp(cmd, "smartstart") == 0)
      {
        ...
        if (!player.isRunning() && config.store.smartstart == 1)
          config.store.smartstart = 0;
        config.save();
        return;
      }
...
{ // %Templates%
  if (var == "ACTION")
    return (network.status == CONNECTED && !config.emptyFS) ? "webboard" : "";
  if (var == "UPLOADWIFI")
...
void handleHTTPArgs(AsyncWebServerRequest *request)
{
  ...
  if (network.status == CONNECTED)
  {
    ...
    if (request->hasArg("volm"))
    {
      player.stepVol(false);
      commandFound = true;
    }
    if (request->hasArg("volp"))
    {
      player.stepVol(true);
      commandFound = true;
    }
    ...
    if (request->hasArg("playstation") || request->hasArg("play"))
    {
      ...
      if (id < 1)
        id = 1;
      if (id > config.store.countStation)
        id = config.store.countStation;
      config.sdResumePos = 0;
